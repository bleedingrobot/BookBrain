...
| 16 | [`16-stage-i-recently-organized-tray.md`](16-stage-i-recently-organized-tray.md) | prompts/15 Stage I | The **last** stage of the accuracy push: `GET /api/library/recently-organized`, a Dashboard tray with Confirm/Correct, `POST /api/files/{id}/confirm`, and an optional `settings.organize_hold_hours` soft-hold (default 0 = no-op). Frontend + API, no AI cost. Needs verifying against a running app. When it lands, do prompts/15 §"When the whole push is done" (fold out dead code, write the "identification pipeline, 2026" section). |
