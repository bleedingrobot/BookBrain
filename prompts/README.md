...
| # | File | Kind | One line |
|---|------|------|----------|
| 17 | [`17-library-viewer-epub-reader.md`](17-library-viewer-epub-reader.md) | feature — `library-viewer` only | In-browser **EPUB reader** (vendored `foliate-js`, no npm dep): full-screen paginated reader, font/theme/TOC controls, IndexedDB offline cache, reading position in `localStorage`, plus a "Continue reading" strip on the home screen. §D word-count→time-left and §E cross-device reading-state sidecar are optional / deferred to `prompts/18`. Ships by pushing `main` (GitHub Actions deploys the viewer). |
